...
async function shortenUrl(req, res) {
    ...
    if (!originalUrl) {
        return res.render('index', { shortUrl: null, qrCodeDataUrl: null, error: 'originalUrl is required' });
    }

    try {
        const shortCode = await urlModel.createShortUrl(originalUrl);
        const shortUrl = `http://localhost:${port}/${shortCode}`;
        const qrCodeDataUrl = await qrcode.toDataURL(shortUrl);
        res.render('index', { shortUrl, qrCodeDataUrl, error: null });
    } catch (error) {
        console.error('Error creating short URL:', error);
        res.render('index', { shortUrl: null, qrCodeDataUrl: null, error: 'Error creating short URL' });
    }
}
